...
class mysql2PDO{
	...
	//removes everything before string.  $include_str = true ->removes everything before and including the string
	// Credit: StackOveflow - http://stackoverflow.com/questions/5329866/remove-everything-before-the-first-in-a-string	
	private function removeBefore($str, $delimiter, $include_str = true){
		if(($pos = strpos($str, $delimiter)) !== false){
		   $new_str = substr($str, $pos + 1);
		}else{
			$this->error .="Error occured: Unable to find delimter \n";		
		}
	}

	//removes everything before string.  $include_str = true ->removes everything before and including the string
	private function removeAfter($str, $delimiter, $include_str = true){
		
		if(strpos($str, $delimiter) !== false){
			$str = substr($str, 0, strpos($str, $delimiter));
	...
	function convertQuery($mysql_query){
		if(strpos($mysql_query,'mysql_query') !== false){
			//Remove everything before and including mysql_query
